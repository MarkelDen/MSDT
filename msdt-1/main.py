...
def update_rmsprop(s,t,params,grads,learning_rate,beta_2=0.999,epsilon=1e-8):
	...
	s_corrected ={}
	for l in range(L):
		s[f"dW{l+1}"] = (s[f"dW{l+1}"]*beta2) + (1-beta2) * np.square(grads[f"dW{l+1}"])
		s[f"db{l+1}"] = (s[f"db{l+1}"]*beta2) + (1-beta2) * np.square(grads[f"db{l+1}"])
		s_corrected[f"dW{l+1}"] = np.divide(s[f"dW{l+1}"],1 - np.power(beta2,t))
		s_corrected[f"db{l+1}"] = np.divide(s[f"db{l+1}"],1 - np.power(beta2,t))
...
def network_model(x_train,y_train,x_test,y_test,learning_rate,epochs,layer_dims,lambd,learning_decay,p_keep,beta,optimizer = None):
	...
	for i in range(epochs):
		...
		if i%50 ==0:
			print("cross entropy loss after {i} th epoch = {cost_train}")
			print("accuracy after {i} th epoch = {score}")
